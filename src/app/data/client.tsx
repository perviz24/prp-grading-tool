    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <div className="mx-auto max-w-7xl px-4 py-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {scars === undefined ? "Loading..." : `${filtered.length} graded scars`}
              </p>
            </div>
          ...
          {scars !== undefined && scars.length > 0 && filtered.length === 0 && (
            <div className="mt-12 text-center">
              <p className="text-lg font-medium">No scars in this group</p>
              <p className="mt-1 text-sm text-muted-foreground">Try changing the filter above</p>
            </div>
          )}
          ...
          {filtered.length > 0 && (
            <div className="mt-6 overflow-x-auto rounded-md border">
              <Table>
                ...
                <TableBody>
                  {filtered.map((scar) => {
                    const acc = predictionAccuracy(scar.predictedOct, scar.actualOct);
                    const timeCategory =
                    ...
                    return (
                      <TableRow key={scar._id}>
                        <TableCell className="font-mono text-sm">{scar.scarCode}</TableCell>
                        <TableCell className="font-mono text-sm">{scar.patientCode}</TableCell>
                        <TableCell>
                          {scar.laserGroup && scar.laserGroup !== "Unknown" ? (
                            <Badge
                              variant={scar.laserGroup === "A-Modern" ? "default" : "secondary"}
                              className="text-xs"
                            >
                        ...
                        <TableCell className="text-xs">{timeCategory ?? "—"}</TableCell>
                        <TableCell className="text-xs">{scar.quadrant} / {scar.zone}</TableCell>
                        <TableCell className="text-center">{scar.fundusGrade}</TableCell>
                        <TableCell className="text-center">{scar.predictedOct}</TableCell>
                        <TableCell className="text-center">{scar.afGrade}</TableCell>
                        <TableCell className="text-center">{scar.revisedOct ?? "—"}</TableCell>
                        <TableCell className="text-center">{scar.actualOct}</TableCell>
